...
8. When you have to inspect or modify files owned by the container (anything under `homeassistant/.storage`, etc.), do it via `docker compose -f docker-compose.yml exec homeassistant sh -c "..."` so the operation runs as the container user. Never try to edit those files from the host using sudo; it will fail and may break permissions.
